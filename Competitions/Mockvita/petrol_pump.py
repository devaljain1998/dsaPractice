...
class Pump:
    """A class whose objects will represent
    the pumps from the cars will fill in the petrol.
    """
    def __init__(self, *args, **kwargs):
        self.quantity = 0
        self.debug_list = []
    
    def fill(self, quantity_of_car: int):
        self.quantity += quantity_of_car
        self.debug_list.append(quantity_of_car)
...
#allocate the pumps:
for capacity in cars:
    if first_pump.quantity <= second_pump.quantity:
        first_pump.fill(capacity)
    else:
        second_pump.fill(capacity)
        
shortest_time = max(first_pump.quantity, second_pump.quantity)
